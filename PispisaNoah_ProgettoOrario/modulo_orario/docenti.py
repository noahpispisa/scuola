...
def docenti(testo,classe):
    ...
    if docenti_classe != []: #se la classe esiste
        file = open("docenti_classi.txt","a")
...
def orario_docente(testo,docente):
    ...
    e crea la tabella dell' orario di un determinato docente e il numero di ore a disposizione
    ...
    try:
        f = open("orario docenti.txt","x+") #prova a creare il file
    except FileExistsError: #se esiste apre l'esistente in modalità lettura
        f = open("orario docenti.txt","r")
    ...
    f = open("orario docenti.txt","a") #apro il file in modalità append
    for i in range(2,len(testo)): #cerco il docente nel testo
        if docente in testo[i][0]: #se è presente inserisco il suo orario 
            for j in range(len(testo[i])):
                f.write(f"{testo[i][j]}")
            f.write("\n")
            while ("   " in testo[i]): #rimuovo gli spazi vuoti
                testo[i].remove("   ")
            f.write(f"Le ore totali del docente sono: {len(testo[i])}\n")
        else:
            f.close()
            return -1
    f.close()
...
def cognome_nome(testo, docente):
    ...
    f=open("cognome e nome.txt", "a")  #apro il file in modalità append
    for i in range(2,len(testo)): #inizio a scorrere la matrice
        if docente in testo[i][0]: #controlla se all'interno della matrice è presente il nome dell' insegnante
            ...
            f.write() #scrivo le ore a disposizione del docente 
    f.close()
    if presente == False:
        return -1
    return
