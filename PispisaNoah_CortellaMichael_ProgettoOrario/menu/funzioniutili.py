...
def i(frase = "Inserisci un intero: "):
    ...
    """
    continua a chiedere un intero finchè non gliene viene dato uno
...
def l(frase,sep = " ",ecc = " ", nomi = False):
    ...
    """
    chiede una striga che verrà poi strippata e splittata, se si specifica che è un insieme di nomi mette le iniziali in maiuscolo
    
    parametri:
    frase: la frase di cortesia
    sep: il separatore(una stringa)
    ecc: "eccesso" ovvero la parte da strippare(stringa)
    nomi: un booleano per capire se è un insieme di nomi o meno
    
    return:
    la lista di stringhe data la stringa iniziale inserita dall'utente
    """
...
def lim_I(frase = "Inserisci un valore: ",min = -10000000,max = 10000000):
...
def lim_R(frase = "Inserisci un valore: ",min = -10000000.0,max = 10000000.0):
